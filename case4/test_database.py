...
def test(args):
    ...
    if args.dataset == 'drugbank':
        ...
        with open('/media/ST-18T/Ma/DualTopoDDI/data/preprocessed/case4/database/drugbank_constructed_database.csv') as f:
            for line in f:
        ...
        mark = 0
        for i,smiles_i in tqdm(approved_drug.items()):
        ...
        for i,smiles_i in tqdm(list(approved_drug.items())[1600:]):
            for j,smiles_j in approved_drug.items():
                for k in range(1316):
        ...
        for i,smiles_i in tqdm(approved_drug.items()):
            for j,smiles_j in approved_drug.items():
                if i != j:
                    test_df.append([i,j,smiles_i,smiles_j,0])
        test_df = pd.DataFrame(test_df, columns=['ID1','ID2','X1','X2', 'Y'])
    ...
    #     else:
    #         for i in range(len(h_id)):
    #             f.write(h_id[i]+','+t_id[i]+','+rel_dict[str(rel_list[i])]+','+str(pred_probs[i])+'\n')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='DeepDDI', help='drugbank, twosides,or DeepDDI')
