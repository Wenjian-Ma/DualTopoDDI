...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--dataset', type=str, required=True, choices=['drugbank', 'twosides','ZhangDDI','ChChMiner','DeepDDI'],
    ...
    args.random_num_gen = np.random.RandomState(args.seed)#随机种子
    if args.operation in ('all', 'drug_data'):
        load_drug_mol_data(args)#将所有药物处理成Graph和Line Graph表示

    if args.operation in ('all','generate_triplets'):
        generate_pair_triplets(args)#整理正样本  生成负样本  保存csv
    
    if args.operation in ('all', 'split'):
        args.class_name = 'Y'
        split_data(args)#根据seed=0划分训练集和测试集，划分三遍——fold=3
